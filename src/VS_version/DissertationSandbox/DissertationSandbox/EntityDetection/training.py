...
def evaluate_new(actual, predicted, mode):
...
def load_doi_questions(location):
    train = pd.read_csv(location + "train.txt", sep="\t", header= None)
    train = DatasetUtils.FormatDOIForEntityDetection(train)

    valid = pd.read_csv(location + "valid.txt", sep="\t", header=None)
    valid = DatasetUtils.FormatDOIForEntityDetection(valid)

    test = pd.read_csv(location + "test.txt", sep="\t", header=None)
    test = DatasetUtils.FormatDOIForEntityDetection(test)
...
def separate_dataset(data, dataset):
    ...
    if dataset == "DOI":
        ...
        data_y = DatasetUtils.FormatDOIForEntitiesOnly(data)
...
def main(args):
    ...
    train_y = DatasetUtils.encode_entities_original(train_y)
    valid_y = DatasetUtils.encode_entities_original(valid_y)
    test_y = DatasetUtils.encode_entities_original(test_y)
